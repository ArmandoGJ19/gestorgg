...
async def predict_spens(current_user: TokenData = Depends(get_current_user)):
    ...
    try:
        ...
        db.predictions.insert_one({"user_id": user_id, "predictions": respuesta_json, "created_at": datetime.now()})
...
@router.get("/finance/predict_spens/last")
async def get_last_prediction(current_user: TokenData = Depends(get_current_user)):
    try:
        if not current_user:
            raise HTTPException(status_code=401, detail="Unauthorized")

        prediction = db.predictions.find_one({"user_id": current_user.user_id}, sort=[("created_at", -1)])
